...
wf["nodes"] = [
    ...
    {"id": "v", "type": "validate", "position": {}, "data": {
        "mode": "rules", "target_column": "nom", "combine": "all", "add_reason": True,
        "rules": [{"test": "char_class", "position": 1, "charclass": "letter"}]}},
]
wf["edges"] = [{"id": "e1", "source": "s", "target": "v", "sourceHandle": "out", "targetHandle": "in"}]
...
print("run 1: source read, cached =", r1["results"]["s"].get("cached"))
...
print("run 2: source CACHED, parquet untouched OK")
...
print("run 3: file changed -> re-read, rows =", r3["results"]["s"]["row_count"])

# force bypass on an unchanged file
mtime3 = pq.stat().st_mtime_ns
time.sleep(0.05)
ran = [ev for ev in engine.iter_run_workflow(pid, wid, only_node="s", force=True)
...
print("run 4: force bypass -> re-read OK")
