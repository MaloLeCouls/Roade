...
export default function DataPreview({
  ...
  useEffect(() => {
    setLoading(true)
    api.preview(pid, wid, nodeId, {
      limit: PAGE, offset, handle,
      sort: sort?.col, dir: sort?.dir, q: q || undefined, filters: filterList,
  ...
  useEffect(() => {
    if (data?.available && !activeColumn && data.columns.length) setActiveColumn(data.columns[0].name)
  }, [data, activeColumn])
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-head">
          ...
          {outputs.length > 1 && (
            <div className="out-switch">
              {outputs.map((o) => (
                <button key={o.handle} className={handle === o.handle ? 'oseg on' : 'oseg'}
                  onClick={() => { setHandle(o.handle); setOffset(0) }}>{o.label}</button>
              ))}
            </div>
          )}
        ...
        {!data ? (
          <div className="preview-main"><div className="preview-content"><p className="muted" style={{ padding: 16 }}>Chargement…</p></div></div>
        ) : !data.available ? (
          <div className="preview-main"><div className="preview-content"><div className="empty">Ce bloc n'a pas encore été exécuté.</div></div></div>
        ) : (
          <>
            <div className="modal-bar">
              <div className="tabs">
                <button className={tab === 'rows' ? 'tab on' : 'tab'} onClick={() => setTab('rows')}>Données</button>
                <button className={tab === 'cols' ? 'tab on' : 'tab'} onClick={() => setTab('cols')}>Colonnes ({data.columns.length})</button>
                <button className={tab === 'stats' ? 'tab on' : 'tab'} onClick={() => setTab('stats')}>Statistiques</button>
                {data.clean_report && <button className={tab === 'clean' ? 'tab on' : 'tab'} onClick={() => setTab('clean')}>Nettoyage</button>}
              </div>
              {tab === 'rows' && (
                <div className="bar-tools">
                  <span className="search-wrap">
                    <span className="si"><Icon name="search" size={13} /></span>
                    <input className="search" placeholder="recherche globale…" value={q}
              ...
              <div className="rowcount">
                ...
                {data.total_count != null && data.total_count !== data.row_count && ` / ${data.total_count.toLocaleString('fr-FR')}`} lignes
              </div>
            </div>
            ...
            <div className="preview-main">
              <div className="preview-content">
                ...
                {tab === 'cols' && (
                  <ColumnsTab columns={data.columns} compiledSql={data.compiled_sql}
                    active={activeColumn} onProfile={setActiveColumn} />
                )}
            ...
            {tab === 'rows' && (
              <div className="modal-foot">
                <button className="ghost" disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - PAGE))}><Icon name="chev-left" /> Précédent</button>
                <span className="muted">lignes {data.row_count === 0 ? 0 : offset + 1}–{Math.min(offset + PAGE, data.row_count)}</span>
                <button className="ghost" disabled={offset + PAGE >= data.row_count} onClick={() => setOffset(offset + PAGE)}>Suivant <Icon name="chev-right" /></button>
              </div>
            )}
          </>
        )}
...
function RowsTable({ columns, rows, sort, onSort, filterOpen, filters, onFilter, onProfile, active }) {
  return (
    <div className="tablewrap">
      <table className="dt">
            ...
            {columns.map((c) => (
              <th key={c.name}>
                <div className="th-row">
                  <span className={`th-name ${active === c.name ? 'active-col' : ''}`} onClick={() => onSort(c.name)} title="Trier">
                    {c.name}
                    {sort?.col === c.name && <span className="sort-ar">{sort.dir === 'asc' ? '▲' : '▼'}</span>}
                  </span>
                  <button className={`th-prof ${active === c.name ? 'on' : ''}`} onClick={() => onProfile(c.name)} title="Profil de la colonne"><Icon name="list" size={12} /></button>
                </div>
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr><td className="idx"></td><td colSpan={columns.length} className="muted" style={{ padding: 16 }}>Aucune ligne.</td></tr>
          ) : rows.map((r, i) => (
            <tr key={i}>
              <td className="idx">{i + 1}</td>
              {columns.map((c) => <td key={c.name}>{fmt(r[c.name])}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="colnames">
      ...
      <ul className="col-list">
        {columns.map((c, i) => (
          <li key={c.name} className={active === c.name ? 'on' : ''} onClick={() => onProfile(c.name)}>
            <span className="cl-idx">{i + 1}</span>
            <span className="cl-name">{c.name}</span>
            <span className="cl-type"><code>{c.type}</code></span>
          </li>
        ))}
      </ul>
    </div>
  ...
  return (
    <div className="tablewrap">
      <table className="dt">
        ...
        <tbody>{stats.map((s, i) => <tr key={i}>{keys.map((k) => <td key={k}>{fmt(s[k])}</td>)}</tr>)}</tbody>
      </table>
    </div>
  )
}
      ...
      {report.map((r, i) => (
        <div className="cr-card" key={i}>
          <div className="cr-head">
            ...
            <span className="cr-col">sur <b>{r.column}</b></span>
          </div>
          <div className="cr-metrics">
            <span className="cr-m"><b>{r.changed.toLocaleString('fr-FR')}</b> modifiées</span>
            {r.was_null > 0 && <span className="cr-m"><b>{r.was_null.toLocaleString('fr-FR')}</b> vides</span>}
            {r.failed > 0 && <span className="cr-m err"><b>{r.failed.toLocaleString('fr-FR')}</b> échecs (→ vide)</span>}
          </div>
          {r.samples?.length > 0 && (
            <table className="cr-samples">
              <thead><tr><th>avant</th><th></th><th>après</th></tr></thead>
              <tbody>
                {r.samples.map((s, j) => (
                  <tr key={j}><td>{s.old ?? <span className="null">vide</span>}</td><td>→</td><td>{s.new ?? <span className="null">vide</span>}</td></tr>
                ))}
              </tbody>
            </table>
          )}
...
function ProfilePanel({ pid, wid, nodeId, handle, column }) {
  ...
  return (
    <div className="profile-panel">
      <div className="pp-head"><span className="pp-col">Profil : {column || '—'}</span></div>
      {!column ? <div className="pp-empty">Cliquez une colonne pour l'analyser.</div>
        : !p ? <div className="pp-empty">Analyse…</div>
        : !p.available ? <div className="pp-empty">Indisponible.</div>
        : (
          <div className="pp-body">
            <div className="pp-type"><code>{p.type}</code></div>
...
function Metric({ label, value }) {
  return <div className="metric"><div className="m-val">{value}</div><div className="m-lbl">{label}</div></div>
}
